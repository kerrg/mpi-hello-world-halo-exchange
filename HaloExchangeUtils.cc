...
void initiateHaloExchange(std::vector<double>& current_data,
    ...
    MPI_Irecv(&current_data[0], 1, MPI_DOUBLE, left_neighbor,
              kReservedMessageTag, MPI_COMM_WORLD, &requests[0]);

    MPI_Irecv(&current_data[points_per_rank + 1], 1, MPI_DOUBLE, right_neighbor,
              kReservedMessageTag, MPI_COMM_WORLD, &requests[1]);

    MPI_Isend(&current_data[1], 1, MPI_DOUBLE, left_neighbor,
              kReservedMessageTag, MPI_COMM_WORLD, &requests[2]);
...
std::ofstream initLogFile(int rank) {
    ...
    if (!logfile)
    {
        std::cerr << "Failed to open logfile for rank " << rank << std::endl;
...
void printResults(int world_size, double max_elapsed, const std::vector<double>& global_results) {
    ...
    std::cout << "\n--- Simulation Summary (Total Points: " << total_points << ") ---" << std::endl;
    std::cout << "Execution Time: " << max_elapsed << " seconds" << std::endl;

    std::cout << std::fixed << std::setprecision(1);
    std::cout << "Samples: [Start: " << global_results[0] << "] ... [Mid: "
              << global_results[total_points / 2] << "] ... [End: "
              << global_results[total_points - 1] << "]" << std::endl;
